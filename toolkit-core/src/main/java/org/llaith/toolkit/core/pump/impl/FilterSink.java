...
public class FilterSink<T> implements Sink<T> {
    ...
    public FilterSink(final Sink<T> sink, final Predicate<T> predicate) {
        this.sink = Guard.notNull(sink);
        this.predicate = Guard.notNull(predicate);
    }

    @Override
    public void put(final Chunk<T> chunk) {
        if (chunk != null) this.sink.put(chunk.apply(this.predicate));
    }
